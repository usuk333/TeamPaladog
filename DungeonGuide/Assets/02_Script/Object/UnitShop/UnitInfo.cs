...
namespace UnitShopScene
{
    public class UnitInfo : MonoBehaviour
    {
        ...
        {
            int gold = Convert.ToInt32(GameManager.Instance.FirebaseData.InfoDictionary["Gold"]) - GameManager.Instance.FirebaseData.GetUnitUpgradeGoldByLevel(currentUnit);
            if (gold < 0)
            {
        ...
        public bool CheckHaveUnitPoint()
        {
            ...
            if (point <= 0)
            {
                warningMessage.SetWarningText("증표가 부족합니다!");
                return false;
            }
            else
            {
        ...
        public void UpdateUnitExp()
        {
            ...
            int point = Convert.ToInt32(GameManager.Instance.FirebaseData.InfoDictionary[$"{currentUnit}Points"]) - 1;
        ...
        private void ChangeUnitData(int i)
        {
            ...
            unitAtkGoldText.text = $"공격력 강화\n{GameManager.Instance.FirebaseData.GetUnitUpgradeGoldByLevel(currentUnit)}골드";
        ...
        private void ChangeSkillInfo(int i)
        {
            ...
            {
                case 0:
                    skillInfoText.text = $"전설의 도끼 '파라슈'의 힘으로 상대를 내려쳐({GameManager.Instance.FirebaseData.GetUnitSkillConditionByLevel(currentUnit)})번 째 공격마다 공격력(+150%) 만큼의 피해를 입히고 공격력(+100%) 만큼의 체력을 회복합니다";
                    break;
                case 1:
